...
import { Card, Descriptions, Table, Tag, Typography, Image, Space, Divider } from 'antd'
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <Descriptions bordered column={2}>
          ...
          <Descriptions.Item label="Tanggal Order">
            {record?.tanggalOrder ? dayjs(record.tanggalOrder).format('DD MMMM YYYY, HH:mm') : '-'}
          </Descriptions.Item>
      ...
      <Card>
        ...
        <Table
          ...
          summary={(pageData: ReadonlyArray<{ subtotal: string }>) => {
            const total = pageData.reduce((sum, item) => sum + parseFloat(item.subtotal), 0)
            return (
              <Table.Summary fixed>
                <Table.Summary.Row>
        ...
        >
          ...
          <Table.Column
            ...
            align="right"
            render={(value) => <Text>Rp {parseFloat(value).toLocaleString('id-ID')}</Text>}
          />
          <Table.Column
            title="Jumlah"
            dataIndex="jumlah"
          ...
          <Table.Column
            ...
            dataIndex="subtotal"
            align="right"
            render={(value) => <Text strong>Rp {parseFloat(value).toLocaleString('id-ID')}</Text>}
          />
          <Table.Column
            title="Aksi"
            key="action"

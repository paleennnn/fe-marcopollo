  ...
  const fetchCustomerStats = async () => {
    try {
      ...
      if (success && apiData) {
        ...
        const totalBelanja = completedOrders.reduce((sum: number, order: any) => {
        ...
        try {
          ...
          if (refundResponse.ok) {
            ...
            const refunds = Array.isArray(refundData) ? refundData : refundData.data || []

            // Calculate total approved refunds
            totalRefund = refunds
              .filter((refund: any) => refund.status === 'disetujui')
              .reduce((sum: number, refund: any) => {
                const refundAmount = refund.totalHarga || refund.total_harga || 0
          ...
          console.warn('⚠️ Failed to fetch refunds, continuing without refund data:', refundError)
        }
        ...
        const processedOrders = completedOrders.slice(0, 10).map((order: any) => {
          ...
          return {
            ...
            nomor_order:
              order.nomor_order || order.nomorOrder || order.order_number || `INV-${order.id}`,
            total_harga: orderTotal, // Gunakan nilai yang sudah ditemukan
            tanggal_order:
              order.tanggal_order || order.tanggalOrder || order.order_date || order.created_at,
    ...
    return (
      <div style={{ padding: '24px 0' }}>
        <Card>
          ...
          >
            <Text type="secondary">Fitur ringkasan belanja hanya tersedia untuk akun customer</Text>
          </Empty>
        </Card>
      </div>
    )
      ...
      <div style={{ padding: '24px 0' }}>
        <Card>
          <Empty
            ...
            image={Empty.PRESENTED_IMAGE_SIMPLE}
          >
            <Button type="primary" icon={<ReloadOutlined />} onClick={fetchCustomerStats}>
              Coba Muat Ulang
            </Button>
          </Empty>
        </Card>
...
const CustomerStatsContent = ({
  ...
  const averageOrder = stats.totalOrder > 0 ? Math.round(netSpending / stats.totalOrder) : 0
    ...
    <div style={{ padding: '24px 0' }}>
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="middle">
        ...
        >
          <Space direction="vertical" size="small">
            ...
            <Text type="secondary">Statistik lengkap semua pesanan yang sudah selesai</Text>
          </Space>
      ...
      >
        {stats.recentOrders.length > 0 ? (
          ...
          <Empty description="Belum ada pesanan yang selesai" image={Empty.PRESENTED_IMAGE_SIMPLE}>
            <Text type="secondary">Pesanan yang sudah selesai akan muncul di sini</Text>
            {usingMockData && (
